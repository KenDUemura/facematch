...
class PyFaces:
    def __init__(self,testimg,imgsdir,egfnum,thrsh):
        ...
        self.extn=parts[len(parts) - 1]
        print "to match:",self.testimg," to all ",self.extn," images in directory:",imgsdir        
        self.facet= eigenfaces.FaceRec()
        self.egfnum=self.set_selected_eigenfaces_count(self.egfnum,self.extn)
        print "number of eigenfaces used:",self.egfnum
     
    def findmatch(self):
        self.facet.checkCache(self.imgsdir,self.extn,self.imgnamelist,self.egfnum,self.threshold)
        mindist,matchfile=self.facet.findmatchingimage(self.testimg,self.egfnum,self.threshold)
        if mindist < 1e-10:
            mindist=0
            
        if not matchfile:
            return {'state':False, 'msg':"NOMATCH! try higher threshold"}
        else:
            return {'state':True, 'matches':matchfile, "dist":str(mindist)}
